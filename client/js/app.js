...
class BreadCallApp {
  ...
  async handleRouteChange() {
    ...
    // Handle token-based routes (explicit token in URL takes priority)
    if (token) {
  ...
  async joinRoom(roomId, name = 'User', password = '', autoGenerateToken = true) {
    // Prevent multiple concurrent join attempts
    if (this.isJoining) {
    ...
    // Try to get media before completing join
    // This prevents race conditions where signaling completes before media is ready
    try {
      await this.mediaManager.getUserMedia();
      console.log('[BreadCallApp] Media acquired successfully');
    } catch (error) {
